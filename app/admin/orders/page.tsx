      ...
      <div className="container mx-auto px-4 py-8">
            ...
            {selectedOrders.length > 0 && (
              <div className="flex items-center space-x-2">
                <Select value={bulkAction} onValueChange={setBulkAction}>
                  ...
                  <SelectContent>
                    <SelectItem value="CONFIRMED">Confirm Orders</SelectItem>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4 mb-8">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="text-center">
                <p className="text-2xl font-bold text-purple-600">{orderStats.completed}</p>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="text-center">
                <p className="text-lg font-bold text-green-600">₹{orderStats.totalRevenue.toFixed(0)}</p>
                  ...
                  <TableRow key={order._id}>
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        <Avatar className="h-8 w-8">
                          <AvatarImage
                            src={`/placeholder.svg?height=32&width=32&text=${order.buyerId.username.charAt(0).toUpperCase()}`}
                            alt={order.buyerId.username}
                          />
                          <AvatarFallback>{order.buyerId.username.charAt(0).toUpperCase()}</AvatarFallback>
                        </Avatar>
                        <div>
                          <p className="font-medium text-sm">{order.buyerId.username}</p>
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        {order.orderItems.slice(0, 2).map((item) => (
                          <Image
                            key={item._id}
                            src={item.productId.imageUrl || "/placeholder.svg?height=30&width=30&text=Product"}
                        ...
                        {order.orderItems.length > 2 && (
                          <span className="text-sm text-gray-500">+{order.orderItems.length - 2}</span>
                        )}
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      <Badge className={getPaymentMethodColor(order.payment.method)}>{order.payment.method}</Badge>
                    </TableCell>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          {order.status === "PENDING" && (
                            <DropdownMenuItem onClick={() => handleOrderAction(order._id, "CONFIRMED")}>
                              <CheckCircle className="mr-2 h-4 w-4" />
                              Confirm Order
                            </DropdownMenuItem>
                          )}
                          {order.status === "CONFIRMED" && (
                            <DropdownMenuItem onClick={() => handleOrderAction(order._id, "DELIVERED")}>
        ...
        <Dialog open={isOrderDetailOpen} onOpenChange={setIsOrderDetailOpen}>
          <DialogContent className="sm:max-w-[900px] max-h-[80vh] overflow-y-auto">
              ...
              <Tabs defaultValue="overview" className="w-full">
                  ...
                  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <Badge className={getPaymentMethodColor(selectedOrder.payment.method)}>
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <div className="flex items-center space-x-3 mb-3">
                        <Avatar className="h-10 w-10">
                          <AvatarImage
                            src={`/placeholder.svg?height=40&width=40&text=${selectedOrder.buyerId.username.charAt(0).toUpperCase()}`}
                            alt={selectedOrder.buyerId.username}
                          />
                          <AvatarFallback>{selectedOrder.buyerId.username.charAt(0).toUpperCase()}</AvatarFallback>
                        </Avatar>
                        <div>
                          <p className="font-medium">{selectedOrder.buyerId.username}</p>
                          <p className="text-sm text-gray-600">{selectedOrder.buyerId.email}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="grid md:grid-cols-2 gap-4">
                        <div className="space-y-2 text-sm">
                          ...
                          <div className="flex justify-between">
                            ...
                            <span>₹{selectedOrder.payment.amount.toFixed(2)}</span>
                          </div>
                        </div>
                        <div className="space-y-2 text-sm">
                          <div className="flex items-center space-x-2">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <span>Status: {selectedOrder.payment.status}</span>
                          </div>
                          {selectedOrder.payment.transaction_uuid && (
                            <div className="flex items-center space-x-2">
                              <FileText className="h-4 w-4 text-gray-400" />
                              <span>Transaction: {selectedOrder.payment.transaction_uuid}</span>
                            </div>
                          )}
                          <div className="flex items-center space-x-2">
                  ...
                  <div className="space-y-4">
                    {selectedOrder.orderItems.map((item) => (
                      <div key={item._id} className="flex items-center justify-between p-4 border rounded-lg">
                        <div className="flex items-center space-x-4">
                          <Image
                            src={item.productId.imageUrl || "/placeholder.svg?height=80&width=80&text=Product"}
                            alt={item.productId.name}
                            width={80}
                            height={80}
                            className="rounded object-cover"
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-500">Seller: {item.sellerId.username}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="font-semibold">₹{(item.price * item.quantity).toFixed(2)}</p>
                          <p className="text-sm text-gray-500">Stock: {item.productId.count}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <TabsContent value="shipping" className="space-y-4">
                  <div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      {selectedOrder.shippingInfo.name || selectedOrder.shippingInfo.address ? (
                        <div className="flex items-start space-x-2">
                          ...
                          <div>
                            ...
                            {selectedOrder.shippingInfo.address && <p>{selectedOrder.shippingInfo.address}</p>}
                            {selectedOrder.shippingInfo.city && <p>{selectedOrder.shippingInfo.city}</p>}
                    ...
                    <div className="space-y-3">
                      <div className="flex items-start space-x-4">
                        ...
                        <div className="flex-1 pb-4">
                          <div className="flex items-center justify-between">
                            <Badge className={getStatusColor("PENDING")}>Order Placed</Badge>
                          ...
                          <div className="flex flex-col items-center">
                            <div
                              className={`w-3 h-3 rounded-full ${
                                selectedOrder.status === "CANCELLED" ? "bg-red-500" : "bg-green-500"
                              }`}
                            />
                            {selectedOrder.status !== "CANCELLED" &&
                              selectedOrder.status !== "DELIVERED" &&
                              selectedOrder.status !== "COMPLETED" && <div className="w-px h-8 bg-gray-300 mt-2" />}
                          </div>
                          <div className="flex-1 pb-4">
                            <div className="flex items-center justify-between">
                              <Badge className={getStatusColor(selectedOrder.status)}>{selectedOrder.status}</Badge>
                            ...
                            <p className="text-sm text-gray-700 mt-1">
                              {selectedOrder.status === "CONFIRMED" && "Order has been confirmed by seller"}
                              {selectedOrder.status === "DELIVERED" && "Order has been delivered successfully"}
            ...
            <DialogFooter>
              ...
              {selectedOrder?.status === "PENDING" && (
                <Button onClick={() => handleOrderAction(selectedOrder._id, "CONFIRMED")}>Confirm Order</Button>
              )}
              {selectedOrder?.status === "CONFIRMED" && (
                <Button onClick={() => handleOrderAction(selectedOrder._id, "DELIVERED")}>Mark Delivered</Button>
              )}
            </DialogFooter>
          </DialogContent>
        </Dialog>
